<#
.DESCRIPTION
This PowerShell script is designed to update Plex and its MPV (libmpv) component on Windows systems. It checks for updates, downloads them if available, and installs them while handling the process of stopping and restarting the Plex application.
...
The script handles stopping and restarting the Plex process during updates.
Main Functions:
Get-PlexFileVersion: Retrieves the current version of Plex installed on the system.
Check-PlexUpdate: Checks if there's an update available for Plex.
...
Stop-RestartPlex: Stops or restarts the Plex process.
Update-Plex: Performs the update process for Plex.
...
.PARAMETER Restart
Restart (switch): Used in Stop-RestartPlex to restart Plex after stopping it.
.PARAMETER Force
...
[2024-08-22 21:51:44.164][INFO] You are using a custom modified Plex mpv and not the standard Plex mpv client. [Function: Check-MPVUpdate]
[2024-08-22 21:51:44.193][INFO] No MPV update required.
[2024-08-22 21:51:44.347][SUCCESS] Successfully got data from plex.tv. [Function: Check-PlexUpdate]
[2024-08-22 21:51:44.394][SUCCESS] Plex is up-to-date. [Function: Check-PlexUpdate]
[2024-08-22 21:51:44.395][SUCCESS] Current Plex version: [1.66.1.215] same as latest online version: [1.66.1.215] [Function: Check-PlexUpdate]
[2024-08-22 21:51:44.407][INFO] No Plex update required.
[2024-08-22 21:51:44.411][SUCCESS] Update process completed.
#>
function Get-PlexFileVersion {
    [CmdletBinding()]
    param (
        [string]$LatestOnlineVersion
    )
    $plexPath = Join-Path -Path $localInstallPath -ChildPath 'Plex.exe'
    if (Test-Path $plexPath) {
        try {
            ...
            $versionPattern = "Plex\s+(\d+\.\d+\.\d+\.\d+)-[a-f0-9]+"
        ...
        New-Log "Plex executable not found at the specified path." -Level WARNING
        return [PSCustomObject]@{
            UpdateNeeded = $true
            LocalVersion = $null
        }
...
function Check-PlexUpdate {
    $plexExePath = Join-Path $localInstallPath -ChildPath 'Plex.exe'
    if (-not (Test-Path $plexExePath)) {
        New-Log "Plex not installed. Install manually." -Level WARNING
        return $null
    }
    try {
        $json = Invoke-RestMethod -Uri "https://plex.tv/api/downloads/6.json"
    ...
    $localVersion = Get-PlexFileVersion -LatestOnlineVersion $downloadVersion
    if ($localVersion.UpdateNeeded -eq $false) {
        New-Log "Plex is up-to-date." -Level SUCCESS
        New-Log "Current Plex version: [$($localVersion.LocalVersion)] same as latest online version: [$downloadVersion]" -Level SUCCESS
        return $null
    }
    else {
        New-Log "Plex is NOT up-to-date."
        New-Log "Current Plex version: [$($localVersion.LocalVersion)] is NOT same as latest online version: [$downloadVersion]. Will update!" -Level SUCCESS
        return @{
            Download = $download
...
function Compare-MPVVersion {
    ...
    Write-Host "=== DEBUG Compare-MPVVersion ===" -ForegroundColor Yellow
    Write-Host "Version d'entrée: [$Version]" -ForegroundColor Cyan
    
    # Supprime le "v" initial si présent
    $cleanVersion = $Version -replace '^v', ''
    
    # Pattern complexe (format: X.Y.Z_A-B-C)
    $complexPattern = '(?<minor>\d+)\.(?<patch>\d+)[_-](?<build>\d+)[_-](?<revision>\d+)'
    
    # Pattern simple (format: X.Y.Z)
    $simplePattern = '^(\d+)\.(\d+)\.(\d+)$'
    
    if ($cleanVersion -match $complexPattern) {
        $minor = $matches['minor']
        $patch = $matches['patch']
        $build = $matches['build']
        $revision = $matches['revision']
        
        Write-Host "Pattern complexe trouvé - Minor: $minor, Patch: $patch, Build: $build, Revision: $revision" -ForegroundColor Green
        
        $customVersionString = "$minor.$patch.$build.$revision"
        Write-Host "Version formatée (complexe): [$customVersionString]" -ForegroundColor Green
        return $customVersionString
    }
    elseif ($cleanVersion -match $simplePattern) {
        $major = $matches[1]
        $minor = $matches[2]
        $patch = $matches[3]
        
        Write-Host "Pattern simple trouvé - Major: $major, Minor: $minor, Patch: $patch" -ForegroundColor Green
        
        # Pour les versions simples, on ajoute .0 pour les build et revision
        $customVersionString = "$major.$minor.$patch.0"
        Write-Host "Version formatée (simple): [$customVersionString]" -ForegroundColor Green
        return $customVersionString
    }
    elseif ($cleanVersion -match '-UNKNOWN') {
        $result = ($cleanVersion -split '-')[0]
        Write-Host "Version UNKNOWN détectée, retour: [$result]" -ForegroundColor Magenta
        return $result
    }
    
    Write-Host "Aucun pattern trouvé pour: [$cleanVersion], retour: null" -ForegroundColor Red
    return $null
}
...
function Check-MPVUpdate {
    ...
    if ((Get-Item (Join-Path $localInstallPath "libmpv-2.dll")).Length -lt 50000000) {
        New-Log "You are using the original Plex mpv client."
    }
    else {
        New-Log "You are using a custom modified Plex mpv and not the standard Plex mpv client."
    }
}
function Stop-RestartPlex {
    [CmdletBinding()]
    param(
        [switch]$Restart
    )
    if ($Restart.IsPresent) {
        New-Log "Restarting Plex..."
        try {
            Start-Process -FilePath (Join-Path $localInstallPath "Plex.exe") -WindowStyle Hidden -ErrorAction Stop
            New-Log "Plex successfully restarted!" -Level SUCCESS
        }
        catch {
            New-Log "Something went wrong when restarting Plex." -Level ERROR
        }
        return
    }
    $plexProcess = Get-Process "Plex" -ErrorAction SilentlyContinue
    if ($plexProcess) {
        New-Log "Stopping Plex process..."
        try {
            $plexProcess | Stop-Process -Force -Confirm:$false -ErrorAction Stop
            New-Log "Successfully stopped Plex before the update." -Level SUCCESS
        }
        catch {
            New-Log "Something went wrong when stopping Plex." -Level ERROR
        }
    }
    else {
        New-Log "Plex is not running. Nothing to stop."
    }
}
function Update-Plex {
    ...
    try {
        New-Log "Starting Plex update process..."
        New-Item -Path $tempPath -ItemType Directory -Force | Out-Null
        $plexFilename = Split-Path $Download -Leaf
        New-Log "Downloading Plex update: $plexFilename"
        try {
            $httpclient = [System.Net.Http.HttpClient]::new()
            ...
            $httpclient.Dispose()
            New-Log "Successfully downloaded the new Plex client." -Level SUCCESS
            & $zipTool x (Join-Path $tempPath $plexFilename) ("-o" + (Join-Path $tempPath "app\")) -y | Out-Null
            New-Log "Successfully extraced the Plex update..." -Level SUCCESS
        }
        catch {
            New-Log "Something went wrong when downloading or extracting the new Plex client. Will abort the update." -Level ERROR
            return
        }
        Stop-RestartPlex
        New-Log "Copying updated Plex files..."
        try {
            ...
            New-Log "Plex update completed successfully! New version: [$Version]" -Level SUCCESS
        }
        catch {
            New-Log "Something went wrong during Plex update. Will abort the update." -Level ERROR
            return
        }
        Stop-RestartPlex -Restart
    }
    catch {
        New-Log "An error occurred during the Plex update process." -Level ERROR
    }
    finally {
...
# Global variables
$localInstallPath = "C:\Program Files\Plex\Plex\"
$backupDir = "$localInstallPath" + "Backup"
$tempPath = "$env:TEMP\plex\"
$zipTool = "nanazipc"
#Start update check
...
$plexUpdateInfo = Check-PlexUpdate
if ($plexUpdateInfo) {
    New-Log "Updating Plex..."
    Update-Plex -Download $plexUpdateInfo.Download -Version $plexUpdateInfo.Version
} else {
    New-Log "No Plex update required."
}
